...
exports.serverStates = exports.responseDelays = exports.mockResponses = void 0;
exports.generateMockResponse = generateMockResponse;
exports.generateCommentsOnlyResponse = generateCommentsOnlyResponse;
/**
 * Generate mock response for conversion requests
 */
function generateMockResponse(originalText, fileType = 'text', convertUnits = true) {
    ...
    if (convertUnits) {
        const unitConversions = [
            // Temperature
            { pattern: /(\d+(?:\.\d+)?)\s*°F\b/g, convert: (f) => `${Math.round((f - 32) * 5 / 9)}°C` },
            { pattern: /(\d+(?:\.\d+)?)\s*degrees?\s+fahrenheit/gi, convert: (f) => `${Math.round((f - 32) * 5 / 9)}°C` },
            // Distance
            { pattern: /(\d+(?:\.\d+)?)\s*feet?\b/g, convert: (ft) => `${(ft * 0.3048).toFixed(2)}m` },
            { pattern: /(\d+(?:\.\d+)?)\s*ft\b/g, convert: (ft) => `${(ft * 0.3048).toFixed(2)}m` },
            { pattern: /(\d+(?:\.\d+)?)\s*inches?\b/g, convert: (inch) => `${(inch * 2.54).toFixed(1)}cm` },
            { pattern: /(\d+(?:\.\d+)?)\s*in\b/g, convert: (inch) => `${(inch * 2.54).toFixed(1)}cm` },
            { pattern: /(\d+(?:\.\d+)?)\s*miles?\b/g, convert: (mile) => `${(mile * 1.609).toFixed(2)}km` },
            { pattern: /(\d+(?:\.\d+)?)\s*yards?\b/g, convert: (yard) => `${(yard * 0.914).toFixed(2)}m` },
            // Weight
            { pattern: /(\d+(?:\.\d+)?)\s*pounds?\b/g, convert: (lb) => `${(lb * 0.454).toFixed(2)}kg` },
            { pattern: /(\d+(?:\.\d+)?)\s*lbs?\b/g, convert: (lb) => `${(lb * 0.454).toFixed(2)}kg` },
            { pattern: /(\d+(?:\.\d+)?)\s*ounces?\b/g, convert: (oz) => `${(oz * 28.35).toFixed(1)}g` },
            { pattern: /(\d+(?:\.\d+)?)\s*oz\b/g, convert: (oz) => `${(oz * 28.35).toFixed(1)}g` },
            // Volume
            { pattern: /(\d+(?:\.\d+)?)\s*gallons?\b/g, convert: (gal) => `${(gal * 3.785).toFixed(2)}L` },
            { pattern: /(\d+(?:\.\d+)?)\s*gal\b/g, convert: (gal) => `${(gal * 3.785).toFixed(2)}L` },
            { pattern: /(\d+(?:\.\d+)?)\s*pints?\b/g, convert: (pint) => `${(pint * 0.473).toFixed(2)}L` },
            { pattern: /(\d+(?:\.\d+)?)\s*quarts?\b/g, convert: (qt) => `${(qt * 0.946).toFixed(2)}L` },
            // Pressure
            { pattern: /(\d+(?:\.\d+)?)\s*psi\b/g, convert: (psi) => `${(psi * 6.895).toFixed(1)}kPa` },
            // Area
            { pattern: /(\d+(?:\.\d+)?)\s*sq\s*ft\b/g, convert: (sqft) => `${(sqft * 0.093).toFixed(2)} sq m` },
            { pattern: /(\d+(?:\.\d+)?)\s*acres?\b/g, convert: (acre) => `${(acre * 0.405).toFixed(2)} hectares` }
        ];
 ...
 */
function generateCommentsOnlyResponse(originalText, fileType) {
    let convertedText = originalText;
    let spellingChanges = 0;
    ...
    const convertedLines = lines.map(line => {
        ...
        const isComment = (fileType === 'javascript' || fileType === 'typescript') && (trimmed.startsWith('//') || trimmed.startsWith('/*') || trimmed.includes('*/')) ||
...
exports.serverStates = {
    ...
    slow: { status: 'healthy', responsive: true, delay: exports.responseDelays.slow },
